...
class MERT(nn.Module):
    def __init__(self, freeze_feature_extractor=True):
        ...
        # The MERT custom code reads `config.use_return_dict` (a read-only
        # property that derives from `return_dict`).  In newer transformers
        # it defaults to False, so the model returns a plain tuple and
        # .hidden_states is lost.  Setting return_dict fixes both.
    ...
    def forward(self, input_values):
        ...
        if self.training:
            # Training path, preserved verbatim. The clone().requires_grad_()
            # is load bearing here: MERT itself runs under no_grad, and this is
            # what makes the AudioCAT head above it differentiable with respect
            # to the hidden states while the backbone stays frozen.
            ...
            # Inference path. Same arithmetic, reduced one layer at a time so
            # the [13, B, 750, 768] stack (30 MB per segment at batch 1) is
            # never materialised and never cloned. The autograd plumbing above
            # is dead weight when nothing is being trained.
            #
            # Verified bit-identical to the training path: max |difference|
            # over the returned tensor is 0.0e+00.
            reduced = [
                torch.clamp(F.normalize(h, p=2, dim=-1), -3.0, 3.0).mean(dim=1)
                for h in outputs.hidden_states
            ]
            time_reduced = torch.stack(reduced, dim=1)
